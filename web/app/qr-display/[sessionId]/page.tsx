...
export default function QRDisplayPage() {
  ...
  const generateQRCode = () => {
    ...
    const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(qrContentString)}`
    setQrCode(qrUrl);
  }
    ...
    <div className="h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5 flex items-center justify-center p-4 overflow-hidden">
      <Card className="w-full max-w-4xl m-4">
        <CardHeader className="text-center pb-2">
          ...
          <CardDescription className="text-sm">Scan this code to mark your attendance</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col md:flex-row gap-6 p-6">
          ...
          <div className="flex-1 flex flex-col items-center space-y-4 p-6 bg-muted/30 rounded-lg">
            <div className="p-4 bg-white rounded-lg border shadow-sm">
              {qrCode ? (
                <img src={qrCode} alt="Attendance QR Code" className="w-56 h-56" />
              ) : (
          ...
          <div className="flex-1 flex flex-col">
            <h3 className="font-semibold text-lg mb-4 pb-2 border-b">Session Information</h3>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <Badge variant="outline" className="text-sm">{sessionInfo.code}</Badge>
                </div>
              </div>
              
              <div className="space-y-1">
                <p className="text-sm font-medium text-muted-foreground">Teacher</p>
                <p>{sessionInfo.teacher} <span className="text-muted-foreground text-sm">({sessionInfo.teacherId})</span></p>
              </div>
